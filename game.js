...
let player,platforms=[],enemies=[],items=[],projectiles=[],particles=[],goal,boss,shopOpen=false,paused=false,checkpointX=70,checkpointY=350,rafId=0,camY=0;
const keys={}, WORLD_COUNT=20;
let audioCtx=null, musicGain=null, sfxGain=null, musicTimer=null, victoryTimer=null, musicOn=true, victoryActive=false;
let sfxTimers=new Set(), worldTransitionId=0;
let lastDrawNow=0, lowDetail=false, slowFrames=0;
const musicTracks=[
...
function initAudio(){
 if(!audioCtx){audioCtx=new (window.AudioContext||window.webkitAudioContext)();musicGain=audioCtx.createGain();sfxGain=audioCtx.createGain();musicGain.gain.value=.045;sfxGain.gain.value=.12;musicGain.connect(audioCtx.destination);sfxGain.connect(audioCtx.destination);}
 if(audioCtx.state==='suspended') audioCtx.resume().catch(()=>{});
}
function tone(freq,dur=.1,type='square',gain=.08,bus='sfx'){if(!audioCtx||!musicOn)return;const o=audioCtx.createOscillator(),g=audioCtx.createGain();o.type=type;o.frequency.value=freq;g.gain.setValueAtTime(gain,audioCtx.currentTime);g.gain.exponentialRampToValueAtTime(.001,audioCtx.currentTime+dur);o.connect(g);g.connect(bus==='music'?musicGain:sfxGain);o.start();o.stop(audioCtx.currentTime+dur+.02)}
function clearAudioTimers(){for(const id of sfxTimers)clearTimeout(id);sfxTimers.clear()}
function stopVictoryMusic(){if(victoryTimer){clearInterval(victoryTimer);victoryTimer=null}victoryActive=false}
...
function playMusic(){if(!audioCtx||!musicOn)return;if(audioCtx.state==='suspended')audioCtx.resume().catch(()=>{});stopVictoryMusic();if(musicTimer){clearInterval(musicTimer);musicTimer=null};let i=0;const tr=musicTracks[world%musicTracks.length];musicTimer=setInterval(()=>{if(!running||paused||shopOpen||victoryActive)return;const n=tr.mel[i%tr.mel.length],b=tr.bass[Math.floor(i/2)%tr.bass.length];tone(n,.18,'triangle',.62,'music');if(i%2===0)tone(b,.28,'sine',.26,'music');if(i%4===3)tone(n*2,.07,'square',.12,'music');i++},tr.beat)}
...
function playVictoryMusic(){if(!audioCtx||!musicOn)return;if(audioCtx.state==='suspended')audioCtx.resume().catch(()=>{});stopAllMusic();victoryActive=true;const melody=[523,659,784,1047,784,1047,1319,1047,784,1047,1319,1568];const bass=[131,131,165,196,131,165,196,262];const play=()=>{if(!running||paused||shopOpen||!victoryActive)return;const t=audioCtx.currentTime+.03;melody.forEach((n,i)=>{const at=t+i*.24;victoryToneAt(n,at,.20,'triangle',.75);if(i%2===0)victoryToneAt(n*2,at+.08,.10,'sine',.18)});bass.forEach((n,i)=>victoryToneAt(n,t+i*.48,.42,'sine',.28));};play();victoryTimer=setInterval(play,melody.length*.24*1000+180)}
...
function resize(){const d=Math.min(devicePixelRatio||1,1.5);viewW=Math.min(document.documentElement.clientWidth||innerWidth,1100);viewH=Math.max(320,document.documentElement.clientHeight||innerHeight);c.width=Math.floor(viewW*d);c.height=Math.floor(viewH*d);c.style.width=viewW+'px';c.style.height=viewH+'px';ctx.setTransform(d,0,0,d,0,0)}addEventListener('resize',resize);resize();
function resetWorld(){
 world=Math.max(0,Math.min(WORLD_COUNT-1,Number.isFinite(world)?Math.floor(world):0));
 platforms=[];enemies=[];items=[];projectiles=[];particles=[];cam=0;checkpointX=70;checkpointY=350;timeLeft=180;
 ...
 const bossEmotes=['😡','🐍','💪','😤','💥','🐺','🔥','👑','🐻','🐍','🦍','🐊','🦏','🐺','🐗','🐯','👹','⚡','💪','👑']; boss={x:7900,y:500,w:110,h:100,vx:0,vy:0,alive:true,hp:6,maxHp:6,type:bossKinds[world],name:bossNames[world],emote:bossEmotes[world],hitFlash:0,hitCooldown:0,defeated:false}; goal={x:8080,y:465};
...
function damage(){
 ...
 lives--;player.inv=1.4;player.x=checkpointX;player.y=checkpointY;player.vy=0;player.vx=0;
 if(lives<=0){
...
function update(dt){
 ...
 if(finishCelebration>0||transitionLocked){
   ...
   particles=particles.filter(p=>p.t>0).slice(-300);
   ...
   if(finishCelebration<=0&&finishNextWorld){
     ...
     if(world<WORLD_COUNT-1){
       ...
       if(transitionId===worldTransitionId){playMusic();showWorldMissionBubble();msg('🌍 WORLD '+(world+1)+'/'+WORLD_COUNT+' — '+themes[world].name)}
     }else{
...
if(rect(player,{x:boss.x,y:boss.y,w:boss.w,h:boss.h})&&boss.hitCooldown<=0){if(player.shield>0||player.inv>0){boss.hp=Math.max(0,boss.hp-.5);boss.hitCooldown=.45;boss.hitFlash=.25;player.vy=-10;burst(boss.x+55,boss.y+40,24);sfx('hit');msg(boss.emote+' BOSS HIT! '+Math.max(0,boss.hp).toFixed(1)+' HP LEFT');}else if(player.vy>2&&player.y+player.h-boss.y<30){boss.hp=Math.max(0,boss.hp-.5);boss.hitCooldown=.55;boss.hitFlash=.25;player.vy=-11;score+=300;burst(boss.x+55,boss.y,22);sfx('stomp');msg('💥 '+boss.name+' HIT! '+Math.max(0,boss.hp).toFixed(1)+' HP LEFT');}else{damage();boss.hitCooldown=.55;}}for(const p of projectiles){if(p.t>0&&boss.hitCooldown<=0&&Math.abs(p.x-(boss.x+55))<65&&Math.abs(p.y-(boss.y+45))<65){boss.hp=Math.max(0,boss.hp-.5);boss.hitCooldown=.18;boss.hitFlash=.25;p.t=0;score+=250;burst(boss.x+55,boss.y+45,20);sfx('fire');msg('🔥 BOSS HIT! '+Math.max(0,boss.hp).toFixed(1)+' HP LEFT');}}if(boss.hp<=0){
 ...
 if(player.x>checkpointX+1100){checkpointX=Math.min(7900,Math.floor(player.x/1200)*1200+70);checkpointY=350;msg('CHECKPOINT!')}
 ...
 particles.forEach(p=>{p.x+=p.vx*frameScale;p.y+=p.vy*frameScale;p.vy+=.15*frameScale;p.t-=dt});particles=particles.filter(p=>p.t>0).slice(-300);
...
function draw(){
 const t=themes[world];ctx.clearRect(0,0,viewW,viewH);const g=ctx.createLinearGradient(0,0,0,innerHeight);g.addColorStop(0,t.sky1);g.addColorStop(1,t.sky2);ctx.fillStyle=g;ctx.fillRect(0,0,viewW,viewH);
 ctx.save();ctx.translate(-cam*.12,0);
 // Parallax background adapts automatically on slower devices.
 const layers=lowDetail?1:3, hillCount=lowDetail?8:24;
 ...
 if(!lowDetail){ctx.fillStyle='#ffffff55';for(let i=0;i<10;i++){const x=i*260+80;const y=85+(i%4)*48;ctx.beginPath();ctx.arc(x,y,28,0,7);ctx.arc(x+30,y-10,36,0,7);ctx.arc(x+66,y,25,0,7);ctx.fill();}}
 ctx.restore();
 ...
 // chimney and smoke
 ctx.fillStyle='#9c6954';ctx.fillRect(homeX+169,homeY-50,25,48);ctx.fillStyle='#7779';for(let i=0;i<4;i++){let sy=homeY-65-i*17+Math.sin(performance.now()/500+i)*4;ctx.beginPath();ctx.arc(homeX+181-i*4,sy,8+i*2,0,7);ctx.fill();}
 ...
 if(finishCelebration>0||transitionLocked){
   ctx.save(); const pulse=1+Math.sin(performance.now()/90)*.06;
 ...
 const secs=Math.max(0,Math.ceil(timeLeft));document.getElementById('timer').textContent=Math.floor(secs/60)+':'+String(secs%60).padStart(2,'0');document.getElementById('coins').textContent=coins;document.getElementById('stars').textContent=stars;document.getElementById('lives').textContent=lives;document.getElementById('score').textContent=score;document.getElementById('world').textContent=(world+1)+'/'+WORLD_COUNT;document.getElementById('power').textContent=player?.power==='fire'?'FIRE':'SMALL';document.getElementById('shield').textContent=player?Math.ceil(player.shield):0;
}
...
function drawHero(px,py){
 ...
 const run=Math.sin(performance.now()/75*Math.min(1.8,Math.max(.8,Math.abs(player.vx)/3)));
 ctx.translate(0,-(moving?Math.abs(run)*1.5:0));
 // Ethan is a true side-facing platformer character. His whole body is oriented toward movement.
 const stride=moving&&!crouch?run*6:0;
 // Ethan gets a completely new themed outfit and new shoes in every kingdom.
 const outfit=ethanOutfits[world%ethanOutfits.length];
 // legs / trousers
 ctx.fillStyle=outfit.pants;
 ctx.fillRect(-14+stride,16,11,17);
 ctx.fillRect(3-stride,16,11,17);
 // new themed shoes for this kingdom
 const toe=4*facing;
 ctx.fillStyle=outfit.shoe;
 ctx.fillRect(-18+stride+Math.min(0,toe),30,20,9);
 ctx.fillRect(2-stride+Math.min(0,toe),30,20,9);
 // Extend the toe on the exact side Ethan's eye/nose faces.
 if(facing>0){ctx.fillRect(0+stride,30,4,9);ctx.fillRect(20-stride,30,4,9)}
 else {ctx.fillRect(-22+stride,30,4,9);ctx.fillRect(-2-stride,30,4,9)}
 ctx.fillStyle=outfit.sole;
 ctx.fillRect(-17+stride+Math.min(0,toe),30,18,3);
 ctx.fillRect(3-stride+Math.min(0,toe),30,18,3);
 ctx.fillStyle=outfit.accent;
 ctx.fillRect(-15+stride+Math.min(0,toe),34,14,2);
 ctx.fillRect(5-stride+Math.min(0,toe),34,14,2);
 // torso / themed clothing
 ctx.fillStyle=outfit.shirt;
 if(crouch) ctx.fillRect(-15,2,29,22); else ctx.fillRect(-14,-6,29,30);
 // belt and kingdom-specific chest trim
 ctx.fillStyle=outfit.pants;ctx.fillRect(-15,8,30,5);
 ctx.fillStyle=outfit.accent;ctx.fillRect(7,8,6,6);
 ctx.fillStyle=outfit.accent;ctx.fillRect(-10,-2,18,3);
 ctx.beginPath();ctx.arc(3,4,3.5,0,7);ctx.fill();
 // back arm
 const arm=moving&&!crouch?run*4:0;
 ctx.fillStyle='#c9895b';ctx.beginPath();ctx.arc(-13-arm,5,5.5,0,7);ctx.fill();
 // front arm swings in movement direction
 ctx.fillStyle='#d99b68';ctx.beginPath();ctx.arc(15+arm,5,6,0,7);ctx.fill();
 // neck
 ctx.fillStyle='#d99b68';ctx.fillRect(7,-18,8,8);
 // head, turned toward facing direction
 ctx.fillStyle='#d99b68';ctx.beginPath();ctx.arc(5,-27,17.5,0,7);ctx.fill();
 // ear
 ctx.fillStyle='#c37e4c';ctx.beginPath();ctx.arc(-8,-26,4.5,0,7);ctx.fill();
 // hair cap/back hair
 ctx.fillStyle='#382219';ctx.beginPath();ctx.arc(2,-35,17,Math.PI,Math.PI*2);ctx.fill();
 ctx.fillRect(-13,-35,18,8);
 // red cap with E
 ctx.fillStyle='#e33e38';ctx.beginPath();ctx.arc(2,-41,15,Math.PI,Math.PI*2);ctx.fill();
 ctx.fillRect(-13,-42,30,6);
 ctx.fillStyle='#fff';ctx.font='bold 11px Arial';ctx.fillText('E',3,-36);
 // side-facing face: one visible eye, eyebrow, nose and mouth
 ctx.fillStyle='#222';ctx.beginPath();ctx.arc(13,-28,2.7,0,7);ctx.fill();
 ctx.fillStyle='#382219';ctx.fillRect(11,-33,7,2);
 ctx.fillStyle='#d99b68';ctx.beginPath();ctx.moveTo(20,-27);ctx.lineTo(27,-24);ctx.lineTo(20,-21);ctx.closePath();ctx.fill();
 ctx.fillStyle='#9b503c';ctx.fillRect(15,-19,6,2);
 // Tiny themed outfit badge helps show the wardrobe changed with the kingdom.
 // Outfit name labels are not drawn on Ethan's body.
 // Star shield: a full head-to-toe bubble around Ethan, with animated shining sparkles.
 if(player.shield>0){
   const now=performance.now()/1000;
   const pulse=Math.sin(now*6)*2.5;
   const rx=40+pulse, ry=58+pulse; // extends above cap and below shoes
   ctx.save();
   ctx.globalAlpha=.18;
   ctx.fillStyle='#7ef7ff';ctx.beginPath();ctx.ellipse(2,-3,rx,ry,0,0,Math.PI*2);ctx.fill();
   ctx.globalAlpha=1;
   ctx.shadowColor='#7ef7ff';ctx.shadowBlur=22;
   ctx.strokeStyle='#7ef7ff';ctx.lineWidth=4.5;
   ctx.beginPath();ctx.ellipse(2,-3,rx,ry,0,0,Math.PI*2);ctx.stroke();
   ctx.shadowColor='#ffffff';ctx.shadowBlur=12;
   // Orbiting shine points and four-point star glints.
   for(let i=0;i<7;i++){
     const a=now*2.6+i*Math.PI*2/7;
     const sx=2+Math.cos(a)*rx*.88, sy=-3+Math.sin(a)*ry*.88;
     const r=2+(i%3);
     ctx.fillStyle=i%2?'#ffffff':'#bfffff';
...
function drawFedoraPrison(prisonActive){
 ...
 if(released){ctx.translate(165,-35);ctx.rotate(Math.sin(performance.now()/120)*.04)}
 // hair
...
function drawBoss(b){
 ctx.save();
 const now=performance.now(), t=now/240, pulse=1+Math.sin(t+b.x*.001)*.018, dir=b.face||1;
 ...
 ctx.translate(b.x+55,b.y+60);ctx.scale(dir*1.18,1.18*pulse);
 // aura + grounded shadow
 if(!lowDetail){ctx.globalAlpha=.16+.07*Math.sin(t*2);ctx.fillStyle=glow;ctx.beginPath();ctx.arc(0,-15,63,0,7);ctx.fill();ctx.globalAlpha=1}
 ctx.fillStyle='#0006';ctx.beginPath();ctx.ellipse(0,43,45,9,0,0,7);ctx.fill();
 // armored legs and boots
 for(const x of [-20,12]){ctx.fillStyle=dark;ctx.fillRect(x,8,15,37);ctx.fillStyle=armor;ctx.beginPath();ctx.roundRect(x-3,10,21,20,4);ctx.fill();ctx.fillStyle='#111820';ctx.beginPath();ctx.roundRect(x-5,36,25,11,4);ctx.fill()}
 // torso armor
 ctx.fillStyle=dark;ctx.beginPath();ctx.moveTo(-34,-39);ctx.lineTo(34,-39);ctx.lineTo(27,13);ctx.lineTo(-27,13);ctx.closePath();ctx.fill();
 ctx.fillStyle=armor;ctx.beginPath();ctx.moveTo(-30,-35);ctx.lineTo(0,-45);ctx.lineTo(30,-35);ctx.lineTo(22,4);ctx.lineTo(0,14);ctx.lineTo(-22,4);ctx.closePath();ctx.fill();
 ctx.strokeStyle=glow;ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(0,-41);ctx.lineTo(0,8);ctx.moveTo(-23,-24);ctx.lineTo(23,-24);ctx.stroke();
 // shoulder plates
 ctx.fillStyle=armor;ctx.beginPath();ctx.roundRect(-48,-36,24,18,7);ctx.roundRect(24,-36,24,18,7);ctx.fill();
 // arms + gauntlets
 ctx.strokeStyle=dark;ctx.lineWidth=15;ctx.beginPath();ctx.moveTo(-34,-25);ctx.lineTo(-42,7);ctx.moveTo(34,-25);ctx.lineTo(42,7);ctx.stroke();ctx.fillStyle=armor;ctx.beginPath();ctx.roundRect(-50,-2,17,24,5);ctx.roundRect(33,-2,17,24,5);ctx.fill();
 // helmet
 ctx.fillStyle=dark;ctx.beginPath();ctx.arc(0,-61,29,Math.PI,0);ctx.lineTo(27,-43);ctx.lineTo(18,-28);ctx.lineTo(-18,-28);ctx.lineTo(-27,-43);ctx.closePath();ctx.fill();
 ctx.fillStyle=armor;ctx.beginPath();ctx.arc(0,-60,25,Math.PI,0);ctx.lineTo(22,-44);ctx.lineTo(13,-32);ctx.lineTo(-13,-32);ctx.lineTo(-22,-44);ctx.closePath();ctx.fill();
 // visor inspired by uploaded armored guardian, but original fantasy design
 ctx.fillStyle='#080d13';ctx.beginPath();ctx.moveTo(-19,-58);ctx.lineTo(19,-58);ctx.lineTo(13,-45);ctx.lineTo(-13,-45);ctx.closePath();ctx.fill();
 ctx.shadowColor=glow;ctx.shadowBlur=12;ctx.fillStyle=glow;ctx.fillRect(-14,-54,10,4);ctx.fillRect(4,-54,10,4);ctx.shadowBlur=0;
 // world-specific crown/horns/crest
 const variant=world%5;
 ctx.fillStyle=glow;
 if(variant===0){ctx.beginPath();ctx.moveTo(-20,-75);ctx.lineTo(-10,-94);ctx.lineTo(-3,-76);ctx.lineTo(7,-96);ctx.lineTo(18,-74);ctx.closePath();ctx.fill()}
 else if(variant===1){ctx.beginPath();ctx.moveTo(-19,-72);ctx.quadraticCurveTo(-38,-91,-29,-103);ctx.quadraticCurveTo(-16,-86,-8,-73);ctx.moveTo(19,-72);ctx.quadraticCurveTo(38,-91,29,-103);ctx.quadraticCurveTo(16,-86,8,-73);ctx.fill()}
 else if(variant===2){ctx.beginPath();ctx.moveTo(0,-80);ctx.lineTo(8,-106);ctx.lineTo(15,-79);ctx.closePath();ctx.fill()}
 else if(variant===3){for(let x=-16;x<=16;x+=16){ctx.beginPath();ctx.arc(x,-78,5,0,7);ctx.fill()}}
 else {ctx.beginPath();ctx.moveTo(-24,-75);ctx.lineTo(-15,-92);ctx.lineTo(0,-78);ctx.lineTo(15,-92);ctx.lineTo(24,-75);ctx.closePath();ctx.fill()}
 // energy weapon / staff, alternates by kingdom
 ctx.strokeStyle='#111820';ctx.lineWidth=7;ctx.beginPath();ctx.moveTo(43,8);ctx.lineTo(48,-75);ctx.stroke();ctx.strokeStyle=glow;ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(48,-76);ctx.lineTo(48,-106);ctx.stroke();ctx.fillStyle=glow;ctx.beginPath();ctx.moveTo(48,-111);ctx.lineTo(40,-96);ctx.lineTo(56,-96);ctx.closePath();ctx.fill();
 // boss UI outside body
 ctx.textAlign='center';ctx.font='900 12px Arial';ctx.fillStyle='#fff';ctx.strokeStyle='#111';ctx.lineWidth=3;ctx.strokeText(b.name,0,68);ctx.fillText(b.name,0,68);
 ctx.fillStyle='#111b';ctx.fillRect(-55,-126,110,13);ctx.fillStyle='#fff';ctx.fillRect(-52,-123,104,7);ctx.fillStyle=glow;ctx.fillRect(-52,-123,104*Math.max(0,b.hp/b.maxHp),7);ctx.strokeStyle='#05080d';ctx.lineWidth=2;ctx.strokeRect(-55,-126,110,13);
 ctx.restore();
}
...
function loop(t){
 ...
 try{
   ...
   last=t;
   if(rawDt>.045) slowFrames=Math.min(120,slowFrames+1); else slowFrames=Math.max(0,slowFrames-1);
   lowDetail=slowFrames>12;
...
function start(){hideWorldMissionBubble();if(rafId){cancelAnimationFrame(rafId);rafId=0}stopAllMusic();for(const k in keys)keys[k]=false;lives=3;coins=0;stars=0;score=0;world=0;level=1;timeLeft=180;finishCelebration=0;finishNextWorld=false;finishEmote='🥳';transitionLocked=false;rescueSequence=0;rescueDone=false;shopOpen=false;paused=false;gameState='playing';document.getElementById('world').textContent='1/'+WORLD_COUNT;resetWorld();running=true;document.getElementById('start').style.display='none';showWorldMissionBubble();last=performance.now();playMusic();rafId=requestAnimationFrame(loop)}
document.getElementById('closeMissionBubble').onclick=hideWorldMissionBubble; document.getElementById('play').onclick=()=>{initAudio();start()}; document.getElementById('audioBtn').onclick=()=>{initAudio();musicOn=!musicOn;document.getElementById('audioBtn').textContent=musicOn?'🔊':'🔇';if(musicOn)playMusic();else {stopAllMusic()}};document.getElementById('pauseBtn').onclick=()=>{if(!running)return;paused=!paused;msg(paused?'PAUSED':'RESUMED');if(paused){if(rafId){cancelAnimationFrame(rafId);rafId=0}draw();}else resumeLoop();};
...
addEventListener('visibilitychange',()=>{if(document.hidden&&running){pauseBeforeHide=paused;paused=true;if(rafId){cancelAnimationFrame(rafId);rafId=0}}else if(running){paused=pauseBeforeHide;if(!paused)resumeLoop()}});
addEventListener('pointerdown',e=>{if(!shopOpen)return;const w=Math.min(620,viewW-40),h=430,l=(viewW-w)/2,t=(viewH-h)/2;const cr=c.getBoundingClientRect(),mx=e.clientX-cr.left,my=e.clientY-cr.top;if(mx<l||mx>l+w||my<t||my>t+h){shopOpen=false;resumeLoop();return}for(let i=0;i<5;i++){const yy=t+105+i*58;if(mx>l+w-120&&mx<l+w-38&&my>yy&&my<yy+48){buy(['fire','sprint','heart','star','shield'][i]);return}}});
